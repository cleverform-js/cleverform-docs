...
💡 Event's callback functions/listeners are __OPTIONAL__.

💡 __onError__ and __onSuccess__ callback functions have access to certain paramaters that you can traverse in your callback function block statement. 
...
__onError__ callback function can access three (3) optional parameters provided by CleverForm.
...
__onSuccess__ callback function can access three (3) optional parameters provided by CleverForm.
...
This can be convenient to those who are working with PHP in the server.
...
💡 When a form is submitted with out any error and if the __onSuccess__ callback function is omitted,
