...
Below is a simple way og implementing form validation using  __CleverForm__ constructor function. 
...
const regForm = new CleverForm({
    ...
    formEvents: {
        ...
        onSuccess: function(validatedData) {
            ...
            console.log("Success, Form has No validation error. Submit data in the server (ex: via AJAX , fetch and axios)");
...
You can easily access field's current value in the form at __runtime__ by accessing the Cleverform instance properties.
0
```js
